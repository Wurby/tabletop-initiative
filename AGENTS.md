...
│   │                    # EndCombatModal.jsx
│   ├── items/           # ItemsDrawer.jsx, ItemDetailModal.jsx (edit), ItemViewModal.jsx (read-only),
│   │                    # ItemDisplayModal.jsx (player-facing table reveal) — item tracker
...
- **`combat.display` has a second type: `'item'`**, alongside `'image'`/`'none'`. `pushItemToTable` (`lib/campaign.js`) snapshots the item's fields (`name`, `itemType`, `rarity`, `attunement`, `value`, `weight`, `notes`, `imageUrl`, plus `itemId` as the live-match key) directly into `combat.display`, the same snapshot-not-reference approach `pushImageToTable` already uses for `url`/`label` — so `TableView.jsx` never needs to cross-reference `campaign.items[]`. `ItemDisplayModal.jsx` renders it player-side (self-gates on `display?.type !== 'item'`, same idiom as `ImageModal.jsx`), rendered unconditionally alongside `ImageModal` in `TableView.jsx`. The trigger lives in `ItemViewModal.jsx`'s left pane, right under the header — "Show to Table"/"Shown on table ✓" plus "Clear" when live, mirroring `ImagePreviewModal`'s isLive pattern. This is a *richer* reveal than pushing just the item's raw image (still available separately via the image thumbnail's own `ImagePreviewModal`) — no laser-pointer/marker support for item-type displays, that's scoped to images only.
- **`ItemDisplayModal.jsx` is two-column when the item has an image**: a fixed-width image column on the left with the name/subtitle header absolutely positioned over its top edge (translucent `bg-brand-ink/60` scrim, so the image still shows through), and the value/weight/notes column on the right gets the full card height — deliberately prioritizing text over the image, which used to eat most of the card's vertical space. No-image items collapse to a single column (opaque forest header bar, no overlay, since there's nothing to overlay onto).
- **DM view has a persistent "Showing to table" indicator** (`DMView.jsx`, directly under the header) whenever `campaign.combat.display.type !== 'none'` — works for both `'image'` and `'item'` since it's driven by the same field, with a "Clear" action calling `clearTableDisplay`. This exists specifically so closing `ItemViewModal`/`ImagePreviewModal` (which never auto-clears the table on close — the whole point of pushing is that it persists for players independent of the DM's own modal state) doesn't mean losing track of what's currently live.
- **`ItemViewModal.jsx` also has full image generation** (Generate/Regenerate, "Choose existing" via `ImagePickerModal`, Remove) — same controls as `ItemDetailModal.jsx`, but since the view modal has no Save step, every action persists immediately via a standalone `dmUpdate` on just `imageUrl`, rather than sitting in local state pending a later save. This is the one way `ItemViewModal` writes to Firestore directly instead of routing through `onEdit`/`ItemDetailModal` — a deliberate exception because requiring a full Edit round-trip just to generate art felt like unnecessary friction for what's otherwise a one-click action everywhere else in the app (locations, templates).
...
- **Every dialog-style modal closes on backdrop click.** The pattern: `onClick={onClose}` on the outer `fixed inset-0` overlay div, `onClick={(e) => e.stopPropagation()}` on the content card immediately inside it, so clicks inside the modal don't bubble up and self-close it. Apply this to any new modal by default. Three deliberate exceptions: (1) `LaserPointerModal.jsx` — its entire body *is* the click-to-place-marker drawing surface, so backdrop-click-to-close would fight every marker placement; only its explicit ✕ closes it. (2) `ImageModal.jsx`/`ItemDisplayModal.jsx` — player-facing table displays with no `onClose` prop at all, DM-controlled only via `combat.display`; nothing for a click to call. (3) `ItemsDrawer.jsx`/`TemplatesSidebar.jsx`'s drawers — the backdrop and drawer content are sibling `fixed`-positioned elements, not parent/child, so clicks inside the drawer never bubble into the backdrop's `onClick` in the first place; no `stopPropagation` needed, and `ItemsDrawer`'s backdrop is additionally conditional on `!pinned` (deliberate, from the pin feature). Modals with busy/guard states respect them the same way the explicit close button already did — `ImageGenModal.jsx` only closes on backdrop click when not `busy`, `SplitModal.jsx` only when `canDismiss`.
