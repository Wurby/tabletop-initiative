...
- Closing `ItemViewModal` (or `ImagePreviewModal`) after pushing something to the table gave no way to tell what's still live short of reopening it ✓ — `DMView.jsx` now shows a persistent "Showing to table: {name} · Clear" bar directly under the header, driven by `campaign.combat.display` for both `'image'` and `'item'` types, visible regardless of which drawers/modals are open or closed
- `ItemDisplayModal.jsx` reworked to a two-column layout when the item has an image ✓ — image column on the left with the name/subtitle header absolutely positioned over its top edge (translucent scrim), value/weight/notes column on the right gets the card's full height. Fixes the original layout, where the image ate ~70% of the card's height and left barely enough room for text before requiring scroll. No-image items collapse to a single opaque-header column
- Image generation was only reachable from `ItemDetailModal` (edit) — `ItemViewModal` (view) could only click-to-preview an existing image, no way to generate/choose/remove one without a full Edit round-trip ✓ — `ItemViewModal.jsx` now has the same Generate/Regenerate/"Choose existing"/Remove controls, writing straight to Firestore on each action (no Save step exists there, so nothing sits pending in local state the way it does in the edit modal)
- `get_template`'s description claimed "including its notes," but the response only ever returned each note's id/title, never the body — no way to fetch a template note's full content via MCP, unlike DM notes (`get_dm_note`) and item notes (`get_item`, inline) ✓ — `resolveTarget` (the scope-resolution helper `upsert_note`/`delete_note` already used) is now exported from `mcp/lib/tools/notes.ts` and reused in `get_dm_note`, which takes an optional `scope`/`template_id` (defaulting to `scope: "dm"`, so every existing call with just `id` is unchanged) — matching the same scope convention the write tools already established, rather than adding a separate `get_template_note` tool. `get_template`'s output now also hints at the exact `get_dm_note` call to make when it has notes to fetch
- Initiative units' notes weren't reachable via MCP at all — worse than the template gap above, since `InitiativeUnit` wasn't even typed with `notes`/`noteFolders` in `mcp/lib/campaignAccess.ts`, `get_initiative` didn't list note ids/titles, and `NoteScope` only had `'dm' | 'template'` ✓ — added a third scope, `'unit'`, all the way through: `InitiativeUnit.notes`/`noteFolders` typed, `resolveTarget` gets a `unit_id` param mirroring `template_id` exactly (same "requires X_id" error, same `writeBack` pattern), `NOTE_SCOPE_PROPS` (shared by every note tool's schema) gained `unit_id`, and `get_initiative` now appends compact `— notes: [id] Title, ...` per unit, matching how `get_template`/`get_item` already surface what's fetchable before you fetch it
- Audited every modal in the app (25 files matched a `fixed inset-0` overlay pattern) — most could only be closed via their explicit ✕/Cancel button, not by clicking off the modal ✓ — added `onClick={onClose}` (backdrop) + `onClick={(e) => e.stopPropagation()}` (content card) to 19 of them: `AdminModal`, `Graveyard`/`GraveyardCard`'s `ClearModal`, `ImageGenModal` (respects its existing `busy` guard), `ImagePickerModal`, `EndCombatModal`, `UnitNotesModal`, `ItemDetailModal`, `ItemViewModal`, `LocationWizardModal`, `CampaignMcpModal`, `NoteOpenModal`, `NotesContextModal` (uses `onDone`, not `onClose`), `PartyModal`, `SessionLogModal`, `SplitModal` (only when `canDismiss`), `TemplateGenModal`, `TemplatesSidebar`'s `TemplateModal`, and `DMView`'s lock dialog. Three deliberate exceptions, documented in `AGENTS.md`: `LaserPointerModal` (backdrop click would break marker placement), `ImageModal`/`ItemDisplayModal` (player-facing, no `onClose` to call), and the `ItemsDrawer`/`TemplatesSidebar` drawer backdrops (already correct — sibling elements, not parent/child, so no `stopPropagation` was needed there)
