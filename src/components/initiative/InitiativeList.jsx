...
function headerColor(type) {
  return type === 'party' ? 'bg-brand-forest' : type === 'ally' ? 'bg-brand-rivulet' : 'bg-brand-danger'
}
  ...
  return (
    <div className="flex flex-col gap-1.5 mt-1">
      <div className="flex items-center gap-1.5">
        ...
        {[0, 1, 2].map((i) => (
          <div key={i} className={`w-4 h-4 border ${saves.s[i] ? 'bg-brand-forest border-brand-forest' : 'border-brand-ink/30'}`} />
        ))}
      </div>
      <div className="flex items-center gap-1.5">
        <span className="text-brand-danger text-xs font-normal w-3">✗</span>
        {[0, 1, 2].map((i) => (
          <div key={i} className={`w-4 h-4 border ${saves.f[i] ? 'bg-brand-danger border-brand-danger' : 'border-brand-ink/30'}`} />
        ))}
      </div>
    </div>
  )
...
function Revealed({ show, children }) {
  return show
    ? children
    : <EyeClosed className="text-brand-ink opacity-30" size={12} />
}
          ...
          <ActiveTurnWrapper key={unit.id} isActive={active} type={unit.type}>
            <div className="w-48 h-full min-h-28 bg-brand-mint-dark shadow-card flex flex-col transition-all">
              ...
              <div className={`${headerColor(unit.type)} px-2 py-1.5 flex items-center gap-1.5`}>
                <span className="text-white font-normal text-sm flex-1 min-w-0 truncate">{unit.name}</span>
              ...
              {!isParty && (
                <div className="mx-3 mb-3 relative border border-brand-ink/20 px-2 pt-4 pb-2">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <HealthBar current={unit.hp?.current ?? 0} max={unit.hp?.max ?? 1} temp={unit.hp?.temp ?? 0} showExact={unit.showHp} />
                  </div>
                </div>
              )}
            </div>
