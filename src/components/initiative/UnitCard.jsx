...
function Stepper({ label, labelColor = 'text-brand-forest', value, onChange, onCommit }) {
  ...
  const btn = 'shrink-0 w-6 h-6 text-xs font-normal border border-brand-mint text-brand-ink hover:bg-brand-mint active:bg-brand-mint-dark transition-colors'
  const btnSm = 'shrink-0 w-5 h-6 text-xs font-normal border border-brand-mint text-brand-ink hover:bg-brand-mint active:bg-brand-mint-dark transition-colors'
  return (
...
export default function UnitCard({ unit, onUpdate, onDelete, onKill, isActive = false, onSetActive }) {
  ...
  useEffect(() => { setLocal(unit) }, [unit.id])
  ...
  function set(path, value) {
    const u = path === 'hp.current' ? { ...local, hp: { ...local.hp, current: value } }
  ...
  if (killing) {
    ...
    function confirmKill() {
      ...
      setKilling(false); setKillXp(''); setSelectedCr(null); setCrPage(0)
    }
    return (
      <div className="flex-shrink-0 w-48 h-full bg-brand-mint-dark shadow-card flex flex-col">
        ...
        <div className="flex flex-col gap-2.5 p-3 flex-1">
          <div className="grid grid-cols-3 gap-1">
            {pageEntries.map(({ cr, xp }) => (
              <button
                key={cr}
                onClick={() => { onKill(unit, xp); setKilling(false); setKillXp(''); setSelectedCr(null); setCrPage(0) }}
          ...
          <input
            ...
            onChange={(e) => { setKillXp(e.target.value); setSelectedCr(null) }}
        ...
        <div className="flex border-t border-brand-mint">
          ...
          <button onClick={() => { setKilling(false); setKillXp(''); setSelectedCr(null); setCrPage(0) }}
  ...
  return (
    ...
    <div className={`w-48 h-full min-h-28 bg-brand-mint-dark shadow-card flex flex-col transition-all ${!local.visible ? 'opacity-50' : ''}`}>
      ...
      <div ref={popoverRef} className={`${TYPE_HEADER[local.type] ?? TYPE_HEADER.mob} px-2 py-1.5 flex items-center gap-1.5 relative`}>
      ...
      <div className="px-3 py-2 flex flex-col gap-2 flex-1">
        ...
        {!isParty && (
          <div className="relative border border-brand-ink/20 px-2 pt-3 pb-2">
            <span className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-brand-mint-dark text-brand-forest text-xs font-normal px-1 leading-none">HP</span>
        ...
        {isParty && local.showDeathSaves && (
          <DeathSaves unit={local} onUpdate={(u) => push(u)} />
        )}
