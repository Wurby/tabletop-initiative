...
function Stepper({ label, labelColor = 'text-brand-forest', value, onChange, onCommit }) {
  ...
  return (
    <div className="flex items-center gap-0.5">
      ...
      <button className={btn} onClick={() => onCommit(val - 5)}>−−</button>
      <button className={btnSm} onClick={() => onCommit(val - 1)}>−</button>
      <input
        type="number"
        value={value}
...
function HpBar({ current, max, temp = 0 }) {
  ...
  return (
    <div className="h-2 w-full bg-brand-mint flex">
      ...
      {dispTemp > 0 && (
        <div className="h-2 bg-brand-rivulet/40 transition-all duration-500" style={{ width: `${dispTemp}%` }} />
      )}
    </div>
  )
}
...
export default function UnitCard({
  ...
  useEffect(() => { setLocal(unitRef.current) }, [unit.id, unit.name])

  useEffect(() => {
    if (!showControls) return
    function handleClick(e) {
      if (popoverRef.current && !popoverRef.current.contains(e.target)) setShowControls(false)
    }
  ...
  useEffect(() => {
    ...
    function handleClick(e) {
      if (conditionRef.current && !conditionRef.current.contains(e.target)) setShowConditionPicker(false)
    }
    document.addEventListener('mousedown', handleClick)
    return () => document.removeEventListener('mousedown', handleClick)
  }, [showConditionPicker])

  function push(updated) { setLocal(updated); onUpdate(updated) }

  function set(path, value) {
    const u =
      path === 'hp.current' ? { ...local, hp: { ...local.hp, current: value } }
      : path === 'hp.max' ? { ...local, hp: { ...local.hp, max: value } }
      : path === 'hp.temp' ? { ...local, hp: { ...local.hp, temp: value } }
      : { ...local, [path]: value }
    setLocal(u)
    return u
  }

  function commit(path, value) { onUpdate(set(path, value)) }
  ...
  function addCustom() {
    ...
    if (!name) return
    const current = local.conditions ?? []
    if (!current.includes(name)) commit('conditions', [...current, name])
    setCustomInput('')
  }
  ...
  if (killing) {
    ...
    function confirmKill() {
      ...
      setKilling(false); setKillXp(''); setCrPage(0)
    }
    return (
          ...
          <div className="flex flex-col gap-2.5 p-3 flex-1">
            <div className="grid grid-cols-3 gap-1">
              {pageEntries.map(({ cr, xp }) => (
                <button key={cr}
                  onClick={() => { onKill(unit, xp); setKilling(false); setKillXp(''); setCrPage(0) }}
            ...
            <div className="flex items-center justify-between">
              <button onClick={() => setCrPage((p) => Math.max(0, p - 1))} disabled={crPage === 0}
                className="text-brand-ink/40 hover:text-brand-ink disabled:opacity-20 text-sm px-1 transition-colors">‹</button>
              <span className="text-brand-ink/40 text-xs">{crPage + 1} / {totalPages}</span>
              <button onClick={() => setCrPage((p) => Math.min(totalPages - 1, p + 1))} disabled={crPage === totalPages - 1}
                className="text-brand-ink/40 hover:text-brand-ink disabled:opacity-20 text-sm px-1 transition-colors">›</button>
            </div>
            <input className="bg-white border border-brand-mint-dark px-2 py-1 text-brand-ink text-sm font-normal focus:outline-none focus:ring-2 focus:ring-brand-rivulet w-full"
              type="number" placeholder="XP" value={killXp}
              onChange={(e) => setKillXp(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && confirmKill()} />
          </div>
          <div className="flex border-t border-brand-mint">
            <button onClick={confirmKill}
              className="flex-1 py-2 text-xs font-normal text-white bg-brand-danger hover:bg-brand-danger-dark transition-colors">Confirm</button>
            <button onClick={() => { setKilling(false); setKillXp(''); setCrPage(0) }}
              className="flex-1 py-2 text-xs font-normal text-brand-ink hover:bg-brand-mint transition-colors border-l border-brand-mint">Cancel</button>
          </div>
        </div>
      </ActiveTurnWrapper>
    )
      ...
      <div className={`w-48 h-full min-h-28 bg-brand-mint-dark shadow-card flex flex-col transition-all ${!local.visible ? 'opacity-50' : ''}`}>
        ...
        <div ref={popoverRef}
          ...
          {showControls && (
            <div className="absolute top-full left-0 right-0 z-20 bg-brand-mint-dark border-b border-x border-brand-ink/10 shadow-modal px-3 py-3 flex flex-col gap-2">
              <Stepper label="AC" value={local.ac}
                onChange={(v) => setLocal({ ...local, ac: v })}
                onCommit={(v) => commit('ac', Number(v))} />
              <Stepper label="init" value={local.initiative}
                onChange={(v) => setLocal({ ...local, initiative: v })}
                onCommit={(v) => commit('initiative', Number(v))} />
              <Stepper label="max" labelColor="text-brand-forest" value={local.hp?.max ?? 0}
                onChange={(v) => setLocal({ ...local, hp: { ...local.hp, max: v } })}
                onCommit={(v) => commit('hp.max', Math.max(0, Number(v)))} />
              {hasTie && (
                <div className="flex items-center gap-2 border-t border-brand-ink/10 pt-2 mt-1">
                  <span className="text-brand-ink/50 text-xs w-7 shrink-0">order</span>
        ...
        <div className="px-3 py-2 flex flex-col gap-2 flex-1">
          ...
          <div className="relative" ref={conditionRef}>
            <div className="flex flex-wrap gap-1 items-center min-h-4">
              {(local.conditions ?? []).map((c) => (
                <button key={c} onClick={() => toggleCondition(c)}
                  className="flex items-center gap-0.5 px-1 py-0.5 text-[10px] leading-none bg-brand-rivulet/10 text-brand-rivulet border border-brand-rivulet/25 hover:bg-brand-rivulet/20 transition-colors">
                  {c}<span className="text-brand-rivulet/50 ml-0.5">×</span>
                </button>
              ))}
              <button
                onClick={() => { setShowConditionPicker((v) => !v); setShowControls(false) }}
                className={`w-4 h-4 flex items-center justify-center text-xs border border-dashed transition-colors ${showConditionPicker ? 'border-brand-rivulet text-brand-rivulet' : 'border-brand-ink/20 text-brand-ink/30 hover:border-brand-ink/40 hover:text-brand-ink/50'}`}>
                +
              </button>
            </div>
            {showConditionPicker && (
              <div className="absolute top-full left-0 z-30 w-56 bg-brand-mint-dark border border-brand-ink/10 shadow-modal p-2 flex flex-col gap-2 mt-1">
                <div className="grid grid-cols-3 gap-1">
                  {CONDITIONS_5E.map((c) => {
                    const isOn = (local.conditions ?? []).includes(c)
                    return (
                      <button key={c} onClick={() => toggleCondition(c)}
                        className={`px-1 py-0.5 text-[10px] leading-none font-normal border transition-colors ${isOn ? 'bg-brand-rivulet text-white border-brand-rivulet' : 'border-brand-ink/20 text-brand-ink hover:border-brand-rivulet/40'}`}>
                        {c}
                      </button>
                    )
                  })}
                </div>
                <div className="flex gap-1 border-t border-brand-ink/10 pt-2">
                  <input
                    className="flex-1 bg-white border border-brand-ink/20 px-1.5 py-0.5 text-xs text-brand-ink focus:outline-none focus:ring-1 focus:ring-brand-rivulet min-w-0"
                    placeholder="Custom note…" value={customInput}
                    onChange={(e) => setCustomInput(e.target.value)}
                    onKeyDown={(e) => e.key === 'Enter' && addCustom()} />
                  <button onClick={addCustom}
                    className="shrink-0 px-2 py-0.5 text-xs text-brand-rivulet border border-brand-rivulet/40 hover:border-brand-rivulet transition-colors">
                    Add
                  </button>
                </div>
              </div>
          ...
          {!isParty && (
            <div className="relative border border-brand-ink/20 px-2 pt-3 pb-2">
              ...
              <div className="flex flex-col gap-2 items-center">
                <Stepper value={local.hp?.current ?? 0}
                  onChange={(v) => setLocal({ ...local, hp: { ...local.hp, current: v } })}
                  onCommit={(v) => commit('hp.current', Math.max(0, Math.min(Number(v), local.hp?.max ?? 0)))} />
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <Stepper value={local.hp?.temp ?? 0}
                    onChange={(v) => setLocal({ ...local, hp: { ...local.hp, temp: v } })}
                    onCommit={(v) => commit('hp.temp', Number(v))} />
                </div>
                <HpBar current={Number(local.hp?.current ?? 0)} max={Number(local.hp?.max ?? 1)} temp={Number(local.hp?.temp ?? 0)} />
              </div>
            </div>
          )}
        ...
        <div className="border-t border-brand-mint">
          <div className="flex">
            <button onClick={() => push({ ...local, visible: !local.visible })}
              ...
              {local.visible ? <EyeOpen className="text-brand-forest" /> : <EyeClosed className="text-brand-ink opacity-40" />}
            </button>
            {!isParty && (
              <button onClick={() => push({ ...local, showHp: !local.showHp })}
